//
//  TasksTable2ViewController.m
//  studentsInfoWithLoc
//
//  Created by Ramez Hamdi Saeed on 01/04/2024.
//

#import "TasksTable2ViewController.h"
#import "TaskPONSO.h"
...
@interface TasksTable2ViewController ()
{
    NSMutableArray *currentTasks,*doneTasks,*searchItems;
...
@implementation TasksTable2ViewController
...
//    [self saveTasks:doneTasks:DONE];
    ...
    currentTasks=[self getTasks:@"INPROGRESS"];
    [currentTasks addObjectsFromArray:[self getTasks:@"DONE"]];
...
//                [searchItems removeObjectAtIndex:indexPath.row];
                ...
                [tableView deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
                [self saveTasks:currentTasks :(taskToBeDeletedTemp.isInProgress)?@"INPROGRESS":@"DONE"];
}];
...
- (IBAction)search:(id)sender {
    searchItems=[self getTasks:@"INPROGRESS"];
    [searchItems addObjectsFromArray:[self getTasks:@"DONE"]];
